...
RegisterServerEvent("scraps:buywap")
AddEventHandler('scraps:buywap', function(price, hash)
    local source = source
    userid = vRP.getUserId({source})
    if cfg.perm ~= nil then 
        if vRP.hasPermission({userid, cfg.perm}) then
            if vRP.tryPayment({userid, price}) then
                TriggerClientEvent("scraps:givewap", source,  hash)
                vRPclient.notify(source, {"~g~Paid "..cfg.currency..tostring(price)})
            else 
                TriggerClientEvent("scraps:menu", source, false)
...
RegisterServerEvent("scrap:buyarmour")
AddEventHandler('scrap:buyarmour', function(price, level)
    playerid = vRP.getUserId({source})
    if cfg.perm ~= nil then 
        if vRP.hasPermission({playerid, cfg.perm}) then
            if vRP.tryPayment({playerid, price}) then
                TriggerClientEvent("scraps:givearmour", source, level)
                vRPclient.notify(source, {"~g~Paid "..cfg.currency..tostring(price)})
            else 
                TriggerClientEvent("scraps:menu", source, false)
                vRPclient.notify(source, {"~r~Insufficient funds"})
        ...
        if vRP.tryPayment({playerid, price}) then
            ...
            TriggerClientEvent("scraps:armour", source, false)
            vRPclient.notify(source, {"~r~Insufficient funds"})
        end
    end
end)
