...
if (settingsjson.settings.StatusEnabled) {
    setInterval(() => {
        ...
        exports.ghmattimysql.execute("SELECT * FROM `erp_user_moneys`", (result) => {
        ...
        exports.ghmattimysql.execute("SELECT * FROM erp_users WHERE banned = 1", (result) => {
    ...
    if (cmd) {
        if (message.channel.name.includes('bot') || message.channel.name.includes('auction') || cmd.conf.name == 'houseauction' || cmd.conf.name == 'embed' || cmd.conf.name == 'rulechange' || cmd.conf.name == 'auction') {
            ...
            try {
                ...
                let embed = {
                    "title": "Error Occured!",
                    "description": "\nAn error occured. Contact ERP with the code:\n\n```" + err.message + "\n```",
                    "color": 13632027
                }
