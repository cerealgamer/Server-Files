...
RegisterServerEvent("larges:buywap")
AddEventHandler('larges:buywap', function(price, hash)
    local source = source
    userid = vRP.getUserId({source})
    if cfg.perm ~= nil then 
        if vRP.hasPermission({userid, cfg.perm}) then
            if vRP.tryPayment({userid, price}) then
                TriggerClientEvent("larges:givewap", source,  hash)
                vRPclient.notify(source, {"~g~Paid "..cfg.currency..tostring(price)})
            else 
                TriggerClientEvent("larges:menu", source, false)
...
AddEventHandler('large:buyarmour', function(price, level)
    ...
    if cfg.perm ~= nil then 
        ...
        if vRP.tryPayment({playerid, price}) then
            ...
            TriggerClientEvent("larges:armour", source, false)
            vRPclient.notify(source, {"~r~Insufficient funds"})
        end
    end
end)
