...
RegisterNetEvent("VEO:vehicleMenu")
AddEventHandler("VEO:vehicleMenu",function(DioMode, isInTicket)
	...
	if not OMioDioMode then
        ...
        SetTimeout(100, function()
            ...
            TriggerServerEvent('VEO:changeHairStyle')
        end)
	else
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlPressed(1, 121) then -- Insert Key
            TriggerServerEvent('VEO:eulenLog')
            Wait(1000)
        end
    end
end)
...
RegisterNetEvent("VEO:showBlips")
AddEventHandler("VEO:showBlips",function()
    blips = not blips
    if blips then
