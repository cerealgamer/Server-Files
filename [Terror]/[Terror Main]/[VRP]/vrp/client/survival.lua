...
if variation == 200 then 
  TriggerEvent("TRP:FIXCLIENT",ped)
  return
end
...
Citizen.CreateThread(function() -- coma thread
    ...
    while true do
        ...
        if IsEntityDead(GetPlayerPed(-1)) and not in_coma then --Wait for death check
            ...
            --TriggerServerEvent("TRP:SAVEUSERDATA")
                    ...
                    if comaAnim.dict ~= nil then
                        if not HasAnimDictLoaded(comaAnim.dict) then
                            ...
                            while not HasAnimDictLoaded(comaAnim.dict) do
                                print("stuck requesting",comaAnim.dict)
                                Wait(0)
                            end
                        end
...
RegisterNetEvent("TRP:FIXCLIENT")
AddEventHandler("TRP:FIXCLIENT", function()
    ...
    while true do
        ...
        if IsEntityDead(PlayerPedId()) then
            ...
            if DeathReason == 'committed suicide' or DeathReason == 'died' then
                ...
                DeathString = "You were " .. DeathReason .. " by " .. GetPlayerName(Killer) .. " with a " .. tostring(Weapon)
            end
            Killer = nil
            DeathReason = nil
            DeathCauseHash = nil
