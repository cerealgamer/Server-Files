...
AddEventHandler('vRPAdmin:PedCleanup', function()
    ...
    if vRP.hasPermission(user_id, 'player.pedcleanup') then
        ...
        TriggerClientEvent('chat:addMessage', -1, {
            color = { 255, 0, 0},
            multiline = true,
            args = {"[SERVER]", GetPlayerName(source) .. ' is cheating! He\'s triggering events without permission'}
          })
    end
end)
...
AddEventHandler('vRPAdmin:VehCleanup', function()
    ...
    if vRP.hasPermission(user_id, 'player.pedcleanup') then
        TriggerClientEvent('chat:addMessage', -1, {
            color = { 255, 0, 0},
            multiline = true,
            args = {"[SERVER]", "A vehicle cleanup has been Triggered, please wait 10 seconds."}
          })
          Wait(10000)
          TriggerClientEvent('chat:addMessage', -1, {
            color = { 255, 0, 0},
            multiline = true,
            args = {"[SERVER]", "A vehicle cleanup will be complete in 5 seconds."}
          })
          Wait(0)
          ...
          TriggerClientEvent('chat:addMessage', -1, {
            color = { 255, 0, 0},
            multiline = true,
            args = {"[SERVER]", "Vehicle Cleanup Completed"}
          })
        else 
        print(GetPlayerName(source) .. ' is cheating! He\'s triggering events without permission')
    end
...
RegisterServerEvent("vRPAdmin:Revive")
AddEventHandler("vRPAdmin:Revive",function(id)
    ...
    if vRP.hasPermission(userid, 'player.revive') then
        ...
        TriggerClientEvent("TRP:FIXCLIENT", SelectedPlrSource)
        vRPclient.notify(SelectedPlrSource, {"~g~You have been revived"})
        vRPclient.notify(source, {"~g~You Revived Perm ID: "..SelectedPlrSource})
        end
    end
end)
...
AddEventHandler('vRPAdmin:Kick', function(id, reason, nof10)
    ...
    if vRP.hasPermission(userid, 'player.kick') then
        if SelectedPlrSource then  
            ...
            vRP.kick(SelectedPlrSource, " You have been kicked for: " .. reason)
