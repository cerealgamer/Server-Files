...
local isStoring = {}
RegisterServerEvent("TRP:Storeweapon")
AddEventHandler("TRP:Storeweapon", function()
    local source = source
    local user_id = vRP.getUserId(source)
    vRPclient.getWeapons(source, {}, function(weapons)
        if not isStoring[source] then
            isStoring[source] = true
            vRPclient.giveWeapons(source, {{}, true}, function(removedwep)
                for k,v in pairs(weapons) do
                    vRP.giveInventoryItem(user_id, "wbody|" .. k, 1, true)
                    if v.ammo > 0 then
                        vRP.giveInventoryItem(user_id, "wammo|" .. k, v.ammo, true)
                ...
                vRPclient.notify(source, {"~g~Weapons Stored"})
                SetTimeout(10000, function()
                    isStoring[source] = nil 
                end)
            end)
        else
            vRPclient.notify(source, {"~o~You are already storing the weapons"})
        end
    end)
end)