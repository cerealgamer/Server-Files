...
print("^5[TerrorRP]: ^7" .. 'Checking for vRP Updates..')
...
if err == 200 then
    ...
    if version ~= r_version then
        print("^5[TerrorRP]: ^7" .. 'A Dunko Update is available from: https://github.com/DunkoUK/dunko_vrp')
    else 
        print("^5[TerrorRP]: ^7" .. 'You are running the most up to date Dunko Version. Thanks for using Dunko_vRP and thanks to our contributors for updating the project. Support Found At: https://discord.gg/b8wQn2XqDt')
    end
else
    print("[TerrorRP] unable to check the remote version")
end
end, "GET", "")--]]
...
Citizen.CreateThread(function()
    ...
    print("[TerrorRP] Customized vRP Framework loaded, customized by cloned and faded.")
end)
...
function vRP.ban(adminsource,permid,time,reason)
    ...
    if getBannedPlayerSrc then 
        if tonumber(time) then 
            ...
            vRP.kick(getBannedPlayerSrc,"You have been banned from this server. Your ban expires in: " .. os.date("%c", banTime) .. " Reason: " .. reason .. " | Banning Admin: " ..  GetPlayerName(adminsource) .. " | ID Of Admin: " .. adminPermID .. " | Your Perm ID is " .. permid) 
            ...
            vRP.kick(getBannedPlayerSrc,"You have been banned from this server. Your ban expires in: " .. "Never, you've been permanently banned." .. " Reason: " .. reason .. " | Banning Admin: " ..  GetPlayerName(adminsource) .. " | ID Of Admin: " .. adminPermID .. " | Your Perm ID is " .. permid) 
        end
    else 
        if tonumber(time) then 
            local banTime = os.time()
            banTime = banTime  + (60 * 60 * tonumber(time))  
            vRPclient.notify(adminsource,{"~g~Success banned! User PermID:" .. permid})
            vRP.setBanned(permid,true,banTime,reason, GetPlayerName(adminsource) .. " | ID Of Admin: " .. adminPermID .. " | Your Perm ID is " .. permid)
        else 
            vRPclient.notify(adminsource,{"~g~Success banned! User PermID:" .. permid})
            vRP.setBanned(permid,true,"perm",reason, GetPlayerName(adminsource) .. " | ID Of Admin: " .. adminPermID .. " | Your Perm ID is " .. permid)
        end
    end
end
...
function vRP.banConsole(permid,time,reason)
    ...
    if getBannedPlayerSrc then 
        if tonumber(time) then 
            local banTime = os.time()
            banTime = banTime  + (60 * 60 * tonumber(time))  
            vRP.setBanned(permid,true,banTime,reason,  'Console' .. " | ID Of Admin: " .. adminPermID .. " | Your Perm ID is " .. permid)
            vRP.kick(getBannedPlayerSrc,"You have been banned from this server. Your ban expires in: " .. os.date("%c", banTime) .. " Reason: " .. reason .. " | Banning Admin: " ..  'Console' .. " | ID Of Admin: " .. adminPermID .. " | Your Perm ID is " .. permid) 
            print("~g~Success banned! User PermID:" .. permid)
        else 
            print("~g~Success banned! User PermID:" .. permid)
            vRP.setBanned(permid,true,"perm",reason,  'Console' .. " | ID Of Admin: " .. adminPermID .. " | Your Perm ID is " .. permid)
            vRP.kick(getBannedPlayerSrc,"You have been banned from this server. Your ban expires in: " .. "Never, you've been permanently banned." .. " Reason: " .. reason .. " | Banning Admin: " ..  'Console' .. " | ID Of Admin: " .. adminPermID .. " | Your Perm ID is " .. permid) 
        end
    else 
        if tonumber(time) then 
            local banTime = os.time()
...
AddEventHandler("playerConnecting",function(name,setMessage, deferrals)
    ...
    if ids ~= nil and #ids > 0 then
        deferrals.update("[TerrorRP] Checking identifiers...")
        vRP.getUserIdByIdentifiers(ids, function(user_id)
            vRP.IdentifierBanCheck(source, user_id, function(status, id)
                if status then
                    print("[TerrorRP] User rejected for attempting to evade ID: " .. user_id .. " | (Ignore joined message, they were rejected)") 
                    deferrals.done("[TerrorRP]: You are banned from this server, please do not try to evade your ban. If you believe this was an error quote your ID which is: " .. id)
                    return 
                end
            end)
            ...
            if user_id ~= nil then -- check user validity 
                deferrals.update("[TerrorRP] Fetching Tokens...")
                vRP.StoreTokens(source, user_id) 
                deferrals.update("[TerrorRP] Checking banned...")
                vRP.isBanned(user_id, function(banned)
                    if not banned then
                        deferrals.update("[TerrorRP] Checking whitelisted...")
                            ...
                            if not config.whitelist or whitelisted then
                                ...
                                if vRP.rusers[user_id] == nil then -- not present on the server, init
                                    if vRP.CheckTokens(source, user_id) then 
                                        deferrals.done("[TerrorRP]: You are banned from this server, please do not try to evade your ban.")
                                    end
                                    vRP.users[ids[1]] = user_id
                                    vRP.rusers[user_id] = ids[1]
                                    vRP.user_tables[user_id] = {}
                                    ...
                                    deferrals.update("[TerrorRP] Loading datatable...")
                                    vRP.getUData(user_id, "vRP:datatable", function(sdata)
                                        local data = json.decode(sdata)
                                        if type(data) == "table" then vRP.user_tables[user_id] = data end
                                        ...
                                        deferrals.update("[TerrorRP] Getting last login...")
                                        vRP.getLastLogin(user_id, function(last_login)
                                            tmpdata.last_login = last_login or ""
                                            tmpdata.spawns = 0
                                ...
                                print("[TerrorRP] "..name.." rejected: not whitelisted (user_id = "..user_id..")")
                                deferrals.done("[TerrorRP] Not whitelisted (user_id = "..user_id..").")
                            end
                        end)
                    else
                        deferrals.update("[TerrorRP] Fetching Tokens...")
                        ...
                        vRP.fetchBanReasonTime(user_id,function(bantime, banreason, banadmin)
                            if tonumber(bantime) then 
                                    ...
                                    if vRP.rusers[user_id] == nil then -- not present on the server, init
                                        ...
                                        vRP.getUData(user_id, "vRP:datatable", function(sdata)
                                            ...
                                            vRP.getLastLogin(user_id, function(last_login)
                                                ...
                                                -- trigger join
                                                print("[TerrorRP] "..name.." joined after his ban expired. (user_id = "..user_id..")")
                                                TriggerEvent("vRP:playerJoin", user_id, source, name, tmpdata.last_login)
                                                deferrals.done()
                                            end)
                                        end)
                                    else -- already connected
                                        print("[TerrorRP] "..name.." re-joined after his ban expired.  (user_id = "..user_id..")")
                                        TriggerEvent("vRP:playerRejoin", user_id, source, name)
                                        deferrals.done()
                                ...
                                print("[TerrorRP] "..name.." rejected: banned (user_id = "..user_id..")")
                                deferrals.done("[TerrorRP] You have been banned from this server.\nYour ban will expire on the: " .. os.date("%c", bantime) .. "\nReason: " .. banreason .. "\n\nBanning Admin: " .. banadmin .. " | Your Perm ID is "..user_id)
                            else 
                                print("[TerrorRP] "..name.." rejected: banned (user_id = "..user_id..")")
                ...
                print("[TerrorRP] "..name.." rejected: identification error")
                deferrals.done("[TerrorRP] Identification error.")
            end
        end)
    else
        print("[TerrorRP] "..name.." rejected: missing identifiers")
        deferrals.done("[TerrorRP] Missing identifiers.")
    end
    Debug.pend()
end)
...
AddEventHandler("playerDropped",function(reason)
    ...
    if user_id ~= nil then
        ...
        print("[TerrorRP] "..vRP.getPlayerEndpoint(source).." disconnected (user_id = "..user_id..")")
